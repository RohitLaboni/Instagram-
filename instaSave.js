...
async function instaSave(url) {
  ...
  if (!el.length) {
    el = $('#download_content .download').first();
  }

  if (!el.length) {
    el = $('.download_content .col-md-4').first();
  }
  ...
  if (el.length) {
    ...
    description =
      el.find('p.text-sm').text().trim() ||
      null;

    profileName =
      el.find('p.text-sm a').text().trim() ||
      null;
